...
- Bước 5.1: Form chọn Môn học & Độ khó (Đã thêm thanh Search)
...
if st.session_state.quiz_phase == "setup":
    ...
    with st.container(border=True):
        ...
        # --- BẮT ĐẦU PHẦN TÌM KIẾM MỚI THÊM VÀO ---
        search_query = st.text_input("🔍 Tìm kiếm môn học", placeholder="Ví dụ: Toán lớp 2, Ngữ pháp...")
        
        # Lọc danh sách môn học dựa trên từ khóa tìm kiếm
        if search_query:
        ...
        # Hiển thị Selectbox với danh sách đã lọc
    ...
    with col1:
        if st.button("Quay về Trang chủ", type="secondary", use_container_width=True):
            ...
            st.switch_page("app.py")
    with col2:
        if st.button("Thử sức với đề mới", type="primary", use_container_width=True):
            st.session_state.quiz_phase = "setup"
            st.session_state.user_answers = {}
            st.session_state.has_saved = False
            st.session_state.question_shuffled_map = {}
